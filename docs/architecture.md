...
- Keyboard handling does **not** use `KeyboardAvoidingView`, because the app runs edge-to-edge on Android. On RN 0.81, `KeyboardAvoidingView` `behavior="height"` in edge-to-edge mode triggers a render loop on Android after the keyboard dismisses (visible as several seconds of jitter), and simply omitting `behavior` hides the composer behind the keyboard because the system no longer resizes the window. `ChatView` and `TerminalScreen` instead use the `useKeyboardInset` hook (`hooks/use-keyboard-inset.ts`), which listens to `Keyboard` events and animates a bottom `paddingBottom` on the screen container via an `Animated.Value`. iOS uses `keyboardWill*` events to track the keyboard; Android uses only `keyboardDid*` events because Android's `will` heights are unreliable and can leave the composer partially covered. On Android RN 0.81 can under-report `endCoordinates.height` (legacy IME detection, fixed upstream only in 0.86), so the hook cross-checks the reported height against the absolute `endCoordinates.screenY` (`windowHeight - screenY`) and pads by the larger estimate so the composer is never left partially covered. The chat screen also lets the user collapse the header + tab row (`ChatView.toggleChrome`) so the transcript gets more room. Visibility is **state-driven**: `chromeHidden` mounts/unmounts the header + tabs block, so the collapse never depends on an animated style actually rendering (RN 0.81 Fabric can silently not apply animated `height`/`opacity` to a layout wrapper, which broke the earlier animated-collapse approach). Hiding fades the block out first (a cosmetic native-driver `chromeFade` opacity animation) and then unmounts after a hard `setTimeout` fallback, so the header is guaranteed to disappear. Showing mounts the block at full opacity. The always-visible floating toggle button (`chromeButtonTop`) auto-adapts to rest just below the header when visible (`chromeTopRef`, measured from the mounted chrome's `onLayout`) and near the status bar when hidden (`insets.top`).
