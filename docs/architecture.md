...
- Keyboard handling does **not** use `KeyboardAvoidingView`, because the app runs edge-to-edge on Android. On RN 0.81, `KeyboardAvoidingView` `behavior="height"` in edge-to-edge mode triggers a render loop on Android after the keyboard dismisses (visible as several seconds of jitter), and simply omitting `behavior` hides the composer behind the keyboard because the system no longer resizes the window. `ChatView` and `TerminalScreen` instead use the `useKeyboardInset` hook (`hooks/use-keyboard-inset.ts`), which listens to the `Keyboard` events and animates a bottom `paddingBottom` on the screen container via an `Animated.Value`. This keeps the composer visible above the keyboard without the `KeyboardAvoidingView` render loop. The chat screen also lets the user collapse the header + tab row (`ChatView.toggleChrome`) so the transcript gets more room.
